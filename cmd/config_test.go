...
`
	...
	// anthropic is now an ordinary provider, no longer a separate legacy block.
	if !strings.Contains(out, "anthropic:") {
		t.Errorf("expected anthropic as a normal provider, got:\n%s", out)
	}
	if strings.Contains(out, "Anthropic (legacy)") {
		t.Errorf("legacy anthropic section should be gone, got:\n%s", out)
	}
	if !strings.Contains(out, "auth_token: ✓ configured (glm-secr..., from yaml)") {
		t.Errorf("expected masked glm auth token, got:\n%s", out)
	}
	if strings.Contains(out, "glm-secret-token") || strings.Contains(out, "deepseek-secret-token") || strings.Contains(out, "sk-ant-abcdef123456") {
